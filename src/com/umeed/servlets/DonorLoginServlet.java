...
public class DonorLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int donorId = DonorDetailsDao.validate(email, password);
		DonorDetails donorDetail = DonorDetailsDao.find(donorId);
		if(donorId != -1) {
			HttpSession session = request.getSession();
			session.setAttribute("donorId", new Integer(donorId));
			session.setAttribute("donorEmail", email);
			session.setAttribute("donorName", donorDetail.getName());
...
//					+ "<button type=\"button\" value=\"\" onclick=\"window.location.href= '/Umeed/Display/DonorLogin.html'; \">Try again</button>"
//					+ "</body>"
//					+ "</html>");
			...
			out.println("<!DOCTYPE html>\r\n" + 
					...
					"    <form action=\"/Umeed/DonorLoginServlet\" method=\"post\" onsubmit=\"return checkPassword(this);\">\r\n" + 
					"		<h2>Login as Donor</h2><p style=\"color:#FF0000\">Email or Password error</p>\r\n" + 
					"        \r\n" + 
					...
					"        <div class=\"text-center\">New to Umeed? <a href=\"/Umeed/Display/DonorRegistration.html\">Sign Up</a></div>\r\n" + 
